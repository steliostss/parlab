...
int main(int argc, char ** argv) {
    ...
    */
    ...
    for (t=0;t<T;t++) {
        ...
        RedSOR(u_previous,u_current,i_min,i_max,j_min,j_max,omega); //check convergence
...
void RedSOR(double ** u_previous, double ** u_current, int X_min, int X_max, int Y_min, int Y_max, int omega) {
    ...
    for (i=X_min;i<X_max;i++)
        for (j=Y_min;j<Y_max;j++)
            if((i+j) % 2 == 0)
                u_current[i][j]=(u_previous[i-1][j]+u_previous[i+1][j]+u_previous[i][j-1]+u_previous[i][j+1] - 4*u_previous[i][j]) * omega / 4.0 + u_previous[i][j];
}
...
void BlackSOR(double ** u_previous, double ** u_current, int X_min, int X_max, int Y_min, int Y_max, int omega) {
    ...
    for (i=X_min;i<X_max;i++)
        for (j=Y_min;j<Y_max;j++)
            if((i+j) % 2 == 1)
                u_current[i][j]=(u_current[i-1][j]+u_current[i+1][j]+u_current[i][j-1]+u_current[i][j+1] - 4*u_previous[i][j]) * omega / 4.0 + u_previous[i][j];
}