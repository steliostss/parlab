...
int main(int argc, char ** argv) {
    ...
    */
    ...
    for (t=0;t<T;t++) {
        ...
        //Swap here so that u_current which becomes an effectively empty space 
        //is the one receiving the t+1 data from neighbors
        swap=u_previous;
...
void GaussSeidel(double ** u_previous, double ** u_current, int X_min, int X_max, int Y_min, int Y_max) {
    ...
    for (i=X_min;i<X_max;i++)
        for (j=Y_min;j<Y_max;j++)
            u_current[i][j] = (u_current[i-1][j] + u_previous[i+1][j] + u_current[i][j-1] + u_previous[i][j+1] - 4 * u_previous[i][j]) * omega / 4.0 + u_previous[i][j];
}